...
export default function Login() {
   ...
   useEffect(() => {
      ...
      document.title = `Green4T | Partner Network`
      ...
      <Box sx={{
         ...
         <Box sx={{
            ...
            <ContentContainer row fullWidth style={{ padding: 0, zIndex: 999 }} gap={0}>
               {smallWidthDevice ? <></> : <CompanyLogo size={30} />}
               <Box sx={{
                  display: 'flex',
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'center', width: smallWidthDevice ? '80%' : '60%' }}>
                     ...
                     <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, alignItems: 'center', width: '100%' }}>
                        <Text title='true' bold='true' style={{ color: '#fff' }}>{`Bem-vindo(a)`}</Text>
                        <Text style={{ color: '#ffffff77' }}>Faça login para acessar o painel.</Text>
                     </Box>
                     <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, width: '100%' }}>
                        <TextInput
                           placeholder='email@green4t.com.br'
                     ...
                     <Button
                        style={{ width: '100%', padding: '12px 20px', backgroundColor: Colors.green, "&:hover": {
...
const CompanyLogo = ({ size = 14 }) => (
   <Box sx={{
      ...
      <img src="/logo_green_partner.svg" alt="Green4T Illustration" style={{ height: `${size}%`, width: 'auto', objectFit: 'contain' }} />
   </Box>
);